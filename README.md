...
```
c2c/          the core mechanism of c2c, as pure functions
scripts/      everything that touches model weights or disk
tests/        the compliance gates
results/      contracts and measurements
assets/       figures
TODO.md       the specification: what each gate claims and how it fails
FINDINGS.md   what the gates measured
```

The split between the first two directories is load-bearing reasons. Because nothing in `c2c/` loads a model, the entire first stage of
checks runs on a machine that has never download the model.
